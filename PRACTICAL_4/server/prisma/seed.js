  ...
  for (let userId = 1; userId <= 10; userId++) {
    for (let j = 1; j <= 5; j++) {
      const video = await prisma.video.create({
        data: {
          userId,
          title: `Video ${j} from user ${userId}`,
          description: `This is a description for video ${j} from user ${userId}`,
          url: `https://example.com/videos/user${userId}_video${j}.mp4`,
          thumbnail: `https://example.com/thumbnails/user${userId}_video${j}.jpg`
        }
      });
      videos.push(video);
  ...
  for (let i = 0; i < 200; i++) {
    ...
    const comment = await prisma.comment.create({
      data: {
        ...
        text: `This is comment ${i + 1}. Lorem ipsum dolor sit amet, consectetur adipiscing elit.`
      }
    });
